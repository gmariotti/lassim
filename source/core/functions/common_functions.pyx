...
cpdef np.ndarray[double] lassim_matrix_function(
    ...
    # sum_mat = -np.dot(np.reshape(k_map, (size, size)), y.T)
    result = np.add(
        np.dot(np.negative(lambdas), y),
        np.dot(vmax, np.reciprocal(
            np.add(ones, np.exp(np.negative(np.dot(k_map, y)))))
               ), out=result
    )
    return result
...
cpdef np.ndarray[double, ndim=2] odeint1e8_lassim_matrix(
    ...
    lambdas = np.diag(sol_vector[:size])
    vmax = np.diag(sol_vector[size: 2 * size])
    k_values = sol_vector[2 * size:]

    # map is a vector, but will be reshaped as a matrix size x size
    k_map[k_map_mask] = k_values
    return integrate.odeint(
        lassim_matrix_function, y0, t,
        args=(lambdas, vmax, np.reshape(k_map, (size, size)), ones, result),
        mxstep=int(1e8)
    )
